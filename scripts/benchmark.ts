...
const CASES: readonly Case[] = [
	{
		label: 'Server log (ISO)',
		arg: 'plaintext',
		build: () =>
			Array.from({ length: 40_000 }, (_, i) =>
				`2026-0${(i % 9) + 1}-1${i % 9}T0${i % 9}:2${i % 6}:1${i % 9}Z level=info request=${i}`,
			).join('\n'),
	},
	{
		label: 'JSON records',
		...
		build: () =>
			JSON.stringify(
				Array.from({ length: 25_000 }, (_, i) => ({
					id: i,
					createdAt: `2026-0${(i % 9) + 1}-1${i % 9}T00:00:00Z`,
				})),
				null,
				1,
			),
	},
	{
		...
		arg: 'csv',
		build: () =>
			`id,date\n${Array.from({ length: 40_000 }, (_, i) => `${i},2026-0${(i % 9) + 1}-1${i % 9}`).join('\n')}`,
	},
];

async function run(content: string, c: Case): Promise<number> {
	return (await extractDates(content, c.arg)).dates.length;
}
