 ...
 * `npx dates-le-mcp` does, minus the registry — so it catches the failures a
...
setTimeout(() => {
	...
	if (!found.includes('2024-03-15T08:30:00Z')) {
		fail(
			'the installed server did not extract a known date',
			`got: ${found.join(', ')}`,
		);
	}
