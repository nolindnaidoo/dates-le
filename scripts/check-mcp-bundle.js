...
send({
	...
	params: {
		name: 'extract_dates',
		arguments: {
			content: '{"created": "2024-03-15T08:30:00Z", "expires": "2025-01-01"}',
			format: 'json',
		},
	},
});
...
setTimeout(() => {
	...
	if (init?.serverInfo?.name !== 'dates-le') {
	...
	if (call?.isError !== false) {
		fail('extract_dates reported an error', JSON.stringify(call));
	}
	const found = call?.structuredContent?.data?.dates?.map((item) => item.value) ?? [];
	// Asserting the actual values, not just "it responded" — a server that
	// answers with an empty list would otherwise pass.
	for (const expected of ['2024-03-15T08:30:00Z', '2025-01-01']) {
		if (!found.includes(expected)) {
			fail(`extract_dates did not find ${expected}`, `got: ${found.join(', ')}`);
		}
	}
