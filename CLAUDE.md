...
- **Extraction is shared with the Rust CLI**, and the corpus under `crate/` is
  ...
  `TZ=America/New_York bun scripts/check-extraction-parity.ts` and updating the corpus — on both
  sides, in the same commit. CI fails when either drifts.
- **What the contract holds equal is the shared `extract_dates` MCP tool**,
