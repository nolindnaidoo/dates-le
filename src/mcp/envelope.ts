 ...
 * `ok` is not `success`.
 *
 * `extractDates` returns `success: false` for an unknown language or an empty
 * run — a true result, not a failure. Reporting that as an error
...
export function toDiagnostics(result: ExtractionResult): readonly Diagnostic[] {
	return result.errors.map((e) => ({
		// The engine's severity scale has three levels; a diagnostic has two,
		// because the only decision a caller makes from it is whether the result
		// is usable. `critical` collapses into `error` rather than becoming a
		// third value nothing branches on — and collapsing it upward, never to a
		// warning, so the worst case can never be reported as the milder one.
