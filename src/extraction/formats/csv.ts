...
// Regex patterns for different date formats in CSV
const ISO_PATTERN =
...
export function extractFromCsv(content: string): DateValue[] {
	...
	const filteredDates = dates.filter((date, _index, arr) => {
		// If this is a custom/simple date, check if it's a substring of an ISO date on the same line
		if (date.format === 'simple') {
