...
// Regex patterns for different date formats in XML
const ISO_PATTERN =
...
/**
 * Extract dates from XML files (Maven POM, build configs, etc.)
 * XML commonly uses ISO 8601 format for timestamps
 */
export function extractFromXml(content: string): DateValue[] {
	...
	lines.forEach((line, lineIndex) => {
		...
		// Extract ISO dates - reset regex lastIndex
		...
		// Extract RFC2822 dates - reset regex lastIndex
		RFC2822_PATTERN.lastIndex = 0;
		...
		// Extract Unix timestamps - reset regex lastIndex
		UNIX_PATTERN.lastIndex = 0;
		...
		// Extract UTC dates - reset regex lastIndex
		UTC_PATTERN.lastIndex = 0;
		...
		// Extract local dates - reset regex lastIndex
		LOCAL_PATTERN.lastIndex = 0;
		...
		// Extract simple dates - reset regex lastIndex
