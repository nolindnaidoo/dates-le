...
//!
//! The extension calls `String.prototype.trim` and matches `\s` in every
//! pattern. Neither means what the Rust equivalent means: JavaScript's
//! whitespace set includes U+FEFF, which Unicode's `White_Space`
//! property does not, and excludes U+0085, which it does. A file
//! beginning with a byte-order mark is ordinary — Notepad, Excel and a
//! PowerShell redirect all add one — so the difference is reachable
//! rather than theoretical, and it reached the shared `extract_dates`
//! tool: `format: "\u{feff}json"` resolved to `json` on one server and
...
//!
//! **This is not V8's date-parser whitespace**, which is a different
//! production and narrower: there a non-breaking space is part of a
//! *word*. `parse.rs` follows the oracle for that and must not be routed
//! through here.
...
/// The same set as a regex character-class body, for the patterns that
/// need `\s`. Held equal to `JS_WHITESPACE` by a test below.
...
mod tests {
    ...
    /// the trimmer and the patterns that use the class.
    #[test]
    fn the_char_list_and_the_regex_class_agree() {
        ...
        for c in JS_WHITESPACE {
            assert!(
                class.is_match(&c.to_string()).unwrap_or(false),
        ...
        for code in 0u32..=0xffff {
            ...
            if class.is_match(&c.to_string()).unwrap_or(false) {
