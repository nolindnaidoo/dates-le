//! The Zed side of Dates-LE.
//!
//! This crate holds no extraction logic. It resolves `dates-le-mcp` from npm and
//! hands Zed a command that launches it over stdio — the same server the VSIX
//! bundles and the same one `npx dates-le-mcp` runs. Keeping it a launcher is
...
const PACKAGE_NAME: &str = "dates-le-mcp";
const SERVER_PATH: &str = "node_modules/dates-le-mcp/server.js";

struct DatesLEExtension;
...
impl zed::Extension for DatesLEExtension {
    fn new() -> Self {
        Self
    }
...
zed::register_extension!(DatesLEExtension);
